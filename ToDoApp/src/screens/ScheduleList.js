...
import { getTaskListsByUserId } from '../database/TaskListDao.js';
import { getDueDateTasksByIds } from '../database/TaskDao.js';
    ...
    useFocusEffect(
        React.useCallback(() => {
            const fetchTasks = async () => {
                ...
                const allTasks = await getDueDateTasksByIds(task_ids);
    ...
    return (
        <View style={styles.container}>
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
                <Text style={styles.subHeader}>Đã lập kế hoạch</Text>
                {tasks.map((task, index) => (
                    <Task key={index} task={task} nameScreen={'Đã lập kế hoạch'} Screen={'ScheduleList'} />
                ))}
